...
private void save() {		//Метод для записи задач в файл. Так же не записывает историю в отдельный файл.
	try (BufferedWriter writer = new BufferedWriter(new FileWriter(taskStorage, StandardCharsets.UTF_8))) {
		writer.write("id,type,name,status,description,epic,startTime,duration"+SEPARATOR);
...
public static FileBackedTaskManager loadFromFile(File file) {	//метод для загрузки данных и истории из файла
	...
	try {
		...
		List<String> taskLineMass = List.of(taskLines.split(SEPARATOR));
		for (int i = 1; i < taskLineMass.size(); i++) {		//если строка пустая, и следующая строка не пустая, формируется список с идентификаторами истории
			...
			switch (task.getTaskType()) { 					//распределение Task-ов по мапам
				case TASK:
...
private String toString(Task task) {	//метод для создания строки из Task
	if (task.getDuration() != null) {
		return String.format("%s,%s,%s,%s,%s,%s,%s,%s"+SEPARATOR, task.getId(), task.getTaskType(), task.getName(),
				task.getStatus(), task.getDescription(), "NoEpic", task.getStartTime(), task.getDuration().toMinutes());
	} else return String.format("%s,%s,%s,%s,%s"+SEPARATOR, task.getId(), task.getTaskType(), task.getName(),
			task.getStatus(), task.getDescription());
}

private String toString(Epic epic) {	//метод для создания строки из Epic
	return String.format("%s,%s,%s,%s,%s"+SEPARATOR, epic.getId(), epic.getTaskType(), epic.getName(),
			epic.getStatus(), epic.getDescription());
}

private String toString(Subtask subtask) {	//метод для создания строки из Subtask
	if (subtask.getDuration() != null) {
		return String.format("%s,%s,%s,%s,%s,%s,%s,%s"+SEPARATOR, subtask.getId(), subtask.getTaskType(), subtask.getName(),
				subtask.getStatus(), subtask.getDescription(), subtask.getEpicID(), subtask.getStartTime(), subtask.getDuration().toMinutes());
	} else return String.format("%s,%s,%s,%s,%s,%s"+SEPARATOR, subtask.getId(), subtask.getTaskType(), subtask.getName(),
...
public static void main(String[] args) {
	...
	manager.addEpic(epic);
		...
		Subtask subtaskOne = new Subtask("SubOne", "Description",Status.NEW,epic.getId());
		Subtask subtaskTwo = new Subtask("SubTwo", "Description",Status.IN_PROGRESS,epic.getId());
		Subtask subtaskThree = new Subtask("SubThree", "Description",Status.DONE,epic.getId());
	...
	Task taskWithTime = new Task("Time", "TimeD", Status.NEW, 30, "2020-10-10T20:20");
	Task taskWithTime2 = new Task("Time2", "TimeD", Status.NEW, 320, "2022-10-10T20:20");
	Subtask subtaskWithTime = new Subtask("Time", "TimeD", Status.NEW, 30, "2010-10-10T20:20", epic.getId());
	Subtask subtaskWithTime2 = new Subtask("Time2", "TimeD", Status.NEW, 120, "2020-10-10T20:20", epic.getId());
	Subtask subtaskWithTime3 = new Subtask("Time3", "TimeD", Status.NEW, 10, "2030-10-10T20:20", epic.getId());
